...
class MediaPanel < Wx::Panel
  def initialize(parent)
	...
	@btn_new_button = Wx::Button.new(self, :label => "PLAY!!!")
	evt_button @btn_new_button,:load_song
	...
	btn_sizer.add(@btn_new_button, 1, Wx::ALL, 2)
  ...
  #I cannot get this to load and play in the same function
  #this only loads the song which is currently hardcoded
  def load_song
  ...
  MEDIA_FILE_WILDCARD = "MP3 Files (*.mp3)|*.mp3|Avi Files (*.avi)|*.avi|MIDI Files (*.midi)|*.mid"
...
class MediaFrame<Wx::Frame
  def initialize()
	...
	#win.do_load_file("bob/res/one.mid")
	#win.do_load_file(File.dirname(__FILE__) + "/../res/one.mid")
	#win.do_load_file(File.expand_path("..", Dir.pwd) + "/res/one.mid")
	#puts (File.expand_path(Dir.pwd) + "/res/one.mid")
